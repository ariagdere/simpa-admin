...
export async function POST({ request }) {
  try {
    ...
    const { productId, variantId, variantCode, values } = body;
    ...
    const id = await saveVariantRow(env.DB, Number(productId), variantId ? Number(variantId) : null, variantCode.trim(), values || {});
