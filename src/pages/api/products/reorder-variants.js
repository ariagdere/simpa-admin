// src/pages/api/products/reorder-variants.js
import { env } from 'cloudflare:workers';
import { reorderProductVariants } from '../../../lib/queries.js';
...
export async function POST({ request }) {
  try {
    ...
    await reorderProductVariants(env.DB, ids.map(Number));
    return new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (err) {
    console.error('Variant reorder error:', err);
